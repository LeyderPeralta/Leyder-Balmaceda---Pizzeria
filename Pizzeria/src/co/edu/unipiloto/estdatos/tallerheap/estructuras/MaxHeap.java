...
public class MaxHeap implements Heap<Pedido> {
    ...
    public void insertar(Pedido p) {
        ...
        while (i > 0 && elementos.get((i - 1) / 2).getPrecio() < elementos.get(i).getPrecio()) {
    ...
    private void heapify(int i) {
        ...
        if (izq < elementos.size() && elementos.get(izq).getPrecio() > elementos.get(mayor).getPrecio()) {
            mayor = izq;
        }
        if (der < elementos.size() && elementos.get(der).getPrecio() > elementos.get(mayor).getPrecio()) {
            mayor = der;
        }
        if (mayor != i) {
            Collections.swap(elementos, i, mayor);
            heapify(mayor);
        }
    }
